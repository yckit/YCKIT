...
<div class="layout">
	...
	<!--{if $comment}-->
	<form action="?action=content&do=comment_delete" method="post" id="comment_form">
	<table cellspacing="0" cellpadding="0" width="100%" class="table">
		...
		<td>
			<b style="color:#88D536"><!--{$comment.name|escape:html}--></b>：<a href="?action=content&do=comment_edit&content_id=<!--{$comment.content_id}-->&comment_id=<!--{$comment.id}-->"><!--{$comment.content|escape:html}--></a>
...
function audit(){
	...
	$('#comment_form').attr("action","?action=content&do=comment_audit");
	$('#comment_form').submit();
}
function del(){
	var status=false;
